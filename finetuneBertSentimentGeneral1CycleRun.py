...
# Load data
#Train
train_data = pd.read_parquet(r"Datasets\Sentiment_General\train.parquet")
if labelType == "coarse":
    train_data = train_data[[ 'text', 'label1']]
    train_data = train_data.rename(columns={"label1":"label"})
else:
    train_data = train_data[[ 'text', 'label2']]
    train_data = train_data.rename(columns={"label2":"label"})
train_data = train_data.rename(columns={"text":"sentence"})
train_data,encoder,classes = Encoder(train_data)
train_dataset = Dataset.from_pandas(train_data)
# Val
val_data = pd.read_parquet(r"..\..\FinetuneData\Sentiment_General\val.parquet")
if labelType == "coarse":
    val_data = val_data[[ 'text', 'label1']]
    val_data = val_data.rename(columns={"label1":"label"})
else:
    val_data = val_data[[ 'text', 'label2']]
    val_data = val_data.rename(columns={"label2":"label"})
val_data = val_data.rename(columns={"text":"sentence"})
val_data = Encoder(val_data,label_encoder=encoder)
val_dataset = Dataset.from_pandas(val_data)
# Test
test_data = pd.read_parquet(r"..\..\FinetuneData\sentiment_General\test.parquet")
if labelType == "coarse":
    test_data = test_data[['text', 'label1']]
    test_data = test_data.rename(columns={"label1":"label"})
else:
    test_data = test_data[[ 'text', 'label2']]
    test_data = test_data.rename(columns={"label2":"label"})
