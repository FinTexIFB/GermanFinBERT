...
def finetuneBertSenitment1Cycle(lr,num_epochs,seed,pathBase,train_dataloader,eval_dataloader, modelPath):
    # Store checkpoint
...
#%% Hyperparameters
seeds=5
learning_rates = [1e-5]#[1e-5,2e-5,3e-5,4e-5,5e-5]
batch_size=8
numEpochs = [5]#[3,4,5]
overallStartTime = time.time()
modelPath = "bert-base-german-cased"#"..\\BaseModels\\german-fin-gbert-optimized-3-fp-512-small-lr-ba10400-converted-hf"#"bert-base-german-cased"
...
# Load data
classes = pd.Series(["negative","neutral","positive"])
#Train
train_data = load_dataset("scherrmann/financial_phrasebank_75agree_german", split="train")
# Val
val_data = load_dataset("scherrmann/financial_phrasebank_75agree_german", split="validation")
# Test
test_data = load_dataset("scherrmann/financial_phrasebank_75agree_german", split="test")
# Tokenize
tokenized_train_dataset = prepare_data(train_data)
tokenized_val_dataset = prepare_data(val_data)
tokenized_test_dataset = prepare_data(test_data)
#%% Finetune
...
for lr in learning_rates:
    for num_epochs in numEpochs:
        pathBase = pathRoot + "\\"+modelName+"\\Sentiment\\lr_"+str(lr)+"num_epochs_"+str(num_epochs)
        ...
        for seed in range(seeds):
            ...
            # Finetune
            finetuneBertSenitment1Cycle(lr,num_epochs,seed,pathBase,train_dataloader,eval_dataloader, modelPath)
...
modelDirs = os.listdir(pathRoot + "\\"+modelName+"\\Sentiment")
modelDirs = [x for x in modelDirs if "num_epochs" in x]
results =[]
for mdl in modelDirs:
    pathAct = pathRoot + "\\"+modelName+"\\Sentiment\\"+mdl
...
modelPath = pathRoot + "\\"+modelName+"\\Sentiment\\lr_"+str(bestLr)+"num_epochs_"+str(bestNumEpoch)
device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
accuracies = {}
precisions = {}
recalls = {}
...
# Store
with open(pathRoot + "\\"+modelName+"\\Sentiment\\test_results.pkl", 'wb') as f:
