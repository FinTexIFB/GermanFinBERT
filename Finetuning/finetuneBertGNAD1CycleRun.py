...
def finetuneBertSenitment1Cycle(lr,num_epochs,seed,pathBase,train_dataloader,eval_dataloader, modelPath, classes):
    # Store checkpoint
...
#%% Hyperparameters
classes = pd.Series(["Web","Panorama","International","Wirtschaft","Sport","Inland","Etat","Wissenschaft","Kultur"])
seeds=5
learning_rates = [1e-5,2e-5,3e-5,4e-5,5e-5]
batch_size=8
numEpochs = [3,4,5]
overallStartTime = time.time()
modelPath = "..\\BaseModels\\german-fin-hf-bert-optimized-continue-ba174000-converted-hf"#"bert-base-german-cased"
...
# Load data
#Train
train_data = load_dataset("gnad10", split='train[:90%]')
train_data = train_data.rename_column("text","sentence")
# Validation
val_data = load_dataset("gnad10", split='train[-10%:]')
val_data = val_data.rename_column("text","sentence")
#Test
test_data = load_dataset("gnad10",split="test")
test_data= test_data.rename_column("text","sentence")
...
for lr in learning_rates:
    for num_epochs in numEpochs:
        pathBase = pathRoot+"\\"+modelName+"\\GNAD\\lr_"+str(lr)+"num_epochs_"+str(num_epochs)
        ...
        for seed in range(seeds):
            ...
            # Finetune
            finetuneBertSenitment1Cycle(lr,num_epochs,seed,pathBase,train_dataloader,eval_dataloader, modelPath,classes)
...
modelDirs = os.listdir(pathRoot+"\\"+modelName+"\\GNAD")
modelDirs = [x for x in modelDirs if "num_epochs" in x]
results =[]
for mdl in modelDirs:
    pathAct = pathRoot+"\\"+modelName+"\\GNAD\\"+mdl
...
modelPath = pathRoot+"\\"+modelName+"\\GNAD\\lr_"+str(bestLr)+"num_epochs_"+str(bestNumEpoch)
device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
accuracies = {}
precisions = {}
recalls = {}
...
# Store
with open(pathRoot+"\\"+modelName+"\\GNAD\\test_results.pkl", 'wb') as f:
